...
if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
	if ($tbl_psat->getCurrentMasterTable() == "tbl_students") {
		$sFilter = $tbl_psat->AddDetailUserIDFilter($sFilter, "tbl_students", $Security->CurrentUserID()); // Add detail User ID filter
		$sDbMasterFilter = $tbl_psat->AddMasterUserIDFilter($sDbMasterFilter, "tbl_students", $Security->CurrentUserID()); // Add master User ID filter
	}
}
if ($sDbDetailFilter <> "") {
...
<p>Student's <span class="edge" style="white-space: nowrap;"> PSAT
</span></p>
<?php if ($tbl_psat->Export == "") { ?>
<?php } ?>
<?php
...
<?php } ?>
		...
		<!-- psat_test_site -->
		<td width="100"<?php echo $tbl_psat->psat_test_site->CellAttributes() ?>>
<?php if ($tbl_psat->RowType == EW_ROWTYPE_EDIT) { // Edit Record ?>
<input type="text" name="x<?php echo $RowIndex ?>_psat_test_site" id="x<?php echo $RowIndex ?>_psat_test_site" title="" size="20" maxlength="125" value="<?php echo $tbl_psat->psat_test_site->EditValue ?>"<?php echo $tbl_psat->psat_test_site->EditAttributes() ?>>
<?php } else { ?>
...
<?php } ?>
		...
		<!-- s_stuid -->
		<td width=""<?php echo $tbl_psat->s_stuid->CellAttributes() ?>>
<?php if ($tbl_psat->RowType == EW_ROWTYPE_EDIT) { // Edit Record ?>
<?php if ($tbl_psat->s_stuid->getSessionValue() <> "") { ?>
<input type="hidden" id="x<?php echo $RowIndex ?>_s_stuid" name="x<?php echo $RowIndex ?>_s_stuid" value="<?php echo ew_HtmlEncode($tbl_psat->s_stuid->CurrentValue) ?>">
<?php } else { ?>
...
function LoadRow() {
	...
	if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
		$sFilter = $tbl_psat->AddDetailUserIDFilter($sFilter, "tbl_students", $Security->CurrentUserID()); // Add User ID filter for master table
	}
...
function AddRow() {
	...
	if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
		$sFilter = $tbl_psat->AddMasterUserIDFilter("", $tbl_psat->getCurrentMasterTable(), $Security->CurrentUserID());
		if ($tbl_psat->getCurrentMasterTable() == "tbl_students") {
