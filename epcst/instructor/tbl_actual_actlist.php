...
if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
	if ($tbl_actual_act->getCurrentMasterTable() == "tbl_students") {
		$sFilter = $tbl_actual_act->AddDetailUserIDFilter($sFilter, "tbl_students", $Security->CurrentUserID()); // Add detail User ID filter
		$sDbMasterFilter = $tbl_actual_act->AddMasterUserIDFilter($sDbMasterFilter, "tbl_students", $Security->CurrentUserID()); // Add master User ID filter
	}
}
if ($sDbDetailFilter <> "") {
...
<?php } ?>
		...
		<!-- a_act_test_site -->
		<td width="100"<?php echo $tbl_actual_act->a_act_test_site->CellAttributes() ?>>
<?php if ($tbl_actual_act->RowType == EW_ROWTYPE_EDIT) { // Edit Record ?>
<input type="text" name="x<?php echo $RowIndex ?>_a_act_test_site" id="x<?php echo $RowIndex ?>_a_act_test_site" title="" size="20" maxlength="125" value="<?php echo $tbl_actual_act->a_act_test_site->EditValue ?>"<?php echo $tbl_actual_act->a_act_test_site->EditAttributes() ?>>
<?php } else { ?>
...
<?php } ?>
		...
		<!-- a_stuid -->
		<td width=""<?php echo $tbl_actual_act->a_stuid->CellAttributes() ?>>
<?php if ($tbl_actual_act->RowType == EW_ROWTYPE_EDIT) { // Edit Record ?>
<?php if ($tbl_actual_act->a_stuid->getSessionValue() <> "") { ?>
<input type="hidden" id="x<?php echo $RowIndex ?>_a_stuid" name="x<?php echo $RowIndex ?>_a_stuid" value="<?php echo ew_HtmlEncode($tbl_actual_act->a_stuid->CurrentValue) ?>">
<?php } else { ?>
...
function LoadRow() {
	...
	if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
		$sFilter = $tbl_actual_act->AddDetailUserIDFilter($sFilter, "tbl_students", $Security->CurrentUserID()); // Add User ID filter for master table
	}
...
function AddRow() {
	...
	if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
		$sFilter = $tbl_actual_act->AddMasterUserIDFilter("", $tbl_actual_act->getCurrentMasterTable(), $Security->CurrentUserID());
		if ($tbl_actual_act->getCurrentMasterTable() == "tbl_students") {
