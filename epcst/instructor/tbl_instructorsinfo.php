...
class ctbl_instructors {
	...
	function ctbl_instructors() {
		...
		$this->g_grpid = new cField('tbl_instructors', 'x_g_grpid', 'g_grpid', "`g_grpid`", 3, -1, FALSE);
	...
	function RenderListRow() {
		...
		// i_instructorid
		$this->i_instructorid->CssStyle = "";
		$this->i_instructorid->CssClass = "";
		$this->i_instructorid->ViewCustomAttributes = "";
		...
		// i_pwd
		$this->i_pwd->ViewValue = "********";
		$this->i_pwd->CssStyle = "";
		$this->i_pwd->CssClass = "";
		$this->i_pwd->ViewCustomAttributes = "";

		// i_instructorid
		$this->i_instructorid->HrefValue = "";
		...
		// i_pwd
		$this->i_pwd->HrefValue = "";
	}

	// User id filter
	function UserIDFilter() {
		return "`i_instructorid`=@UserID@";
	}

	// Add User ID filter
	function AddUserIDFilter($sFilter, $userid) {
		$sFilterWrk = "";
		$sFilterWrk = $this->AddParentUserIDFilter($this->UserIDFilter(), "`i_instructorid`", $userid);
		if ($sFilter <> "") {
			if ($sFilterWrk <> "") {
	...
	function GetUserIDSubquery(&$fld, &$masterfld, $userid) {
		global $conn;
		$sWrk = "";
		$sSql = "SELECT " . $masterfld->FldExpression . " FROM `tbl_instructors` WHERE " . $this->AddUserIDFilter("", $userid);
