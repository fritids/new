...
if (@$_GET["a_actid"] <> "") {
	...
	if (!is_numeric($tbl_actual_act->a_actid->QueryStringValue)) {
		Page_Terminate($tbl_actual_act->getReturnUrl()); // Prevent sql injection, exit
	}
	$sKey .= $tbl_actual_act->a_actid->QueryStringValue;
} else {
	$bSingleDelete = FALSE;
}
if ($bSingleDelete) {
...
if ($nKeySelected <= 0) Page_Terminate($tbl_actual_act->getReturnUrl()); // No key specified, exit
...
foreach ($arRecKeys as $sKey) {
	...
	$sFilter .= "`a_actid`=" . ew_AdjustSql($sKeyFld) . " AND ";
	if (substr($sFilter, -5) == " AND ") $sFilter = substr($sFilter, 0, strlen($sFilter)-5) . ") OR ";
}
if (substr($sFilter, -4) == " OR ") $sFilter = substr($sFilter, 0, strlen($sFilter)-4);

// Set up filter (Sql Where Clause) and get Return Sql
// Sql constructor in tbl_actual_act class, tbl_actual_actinfo.php

$tbl_actual_act->CurrentFilter = $sFilter;

// Get action
if (@$_POST["a_delete"] <> "") {
	$tbl_actual_act->CurrentAction = $_POST["a_delete"];
} else {
	$tbl_actual_act->CurrentAction = "I"; // Display record
}
switch ($tbl_actual_act->CurrentAction) {
	case "D": // Delete
		$tbl_actual_act->SendEmail = TRUE; // Send email on delete success
		if (DeleteRows()) { // delete rows
			$_SESSION[EW_SESSION_MESSAGE] = "Delete Successful"; // Set up success message
			Page_Terminate($tbl_actual_act->getReturnUrl()); // Return to caller
		}
}
...
<p><span class="edge">Delete Student's Actual ACT<br>
    <br><a href="<?php echo $tbl_actual_act->getReturnUrl() ?>">Go Back</a></span></p>
<?php
...
?>
<form action="tbl_actual_actdelete.php" method="post">
<p>
...
while (!$rs->EOF) {
	...
	// Render row value
	$tbl_actual_act->RowType = EW_ROWTYPE_VIEW; // view
	RenderRow();
?>
	<tr<?php echo $tbl_actual_act->DisplayAttributes() ?>>
		<td width="100"<?php echo $tbl_actual_act->a_act_test_date->CellAttributes() ?>>
<div<?php echo $tbl_actual_act->a_act_test_date->ViewAttributes() ?>><?php echo $tbl_actual_act->a_act_test_date->ViewValue ?></div></td>
		<td width="75"<?php echo $tbl_actual_act->a_act_english->CellAttributes() ?>>
<div<?php echo $tbl_actual_act->a_act_english->ViewAttributes() ?>><?php echo $tbl_actual_act->a_act_english->ViewValue ?></div></td>
		<td width="75"<?php echo $tbl_actual_act->a_act_math->CellAttributes() ?>>
<div<?php echo $tbl_actual_act->a_act_math->ViewAttributes() ?>><?php echo $tbl_actual_act->a_act_math->ViewValue ?></div></td>
		<td width="75"<?php echo $tbl_actual_act->a_act_reading->CellAttributes() ?>>
<div<?php echo $tbl_actual_act->a_act_reading->ViewAttributes() ?>><?php echo $tbl_actual_act->a_act_reading->ViewValue ?></div></td>
		<td width="75"<?php echo $tbl_actual_act->a_act_science->CellAttributes() ?>>
<div<?php echo $tbl_actual_act->a_act_science->ViewAttributes() ?>><?php echo $tbl_actual_act->a_act_science->ViewValue ?></div></td>
		<td width="75"<?php echo $tbl_actual_act->a_act_essay->CellAttributes() ?>>
<div<?php echo $tbl_actual_act->a_act_essay->ViewAttributes() ?>><?php echo $tbl_actual_act->a_act_essay->ViewValue ?></div></td>
		<td width="120"<?php echo $tbl_actual_act->a_act_test_site->CellAttributes() ?>>
<div<?php echo $tbl_actual_act->a_act_test_site->ViewAttributes() ?>><?php echo $tbl_actual_act->a_act_test_site->ViewValue ?></div></td>
	</tr>
<?php
	$rs->MoveNext();
}
...
function DeleteRows() {
	...
	$DeleteRows = TRUE;
	$sWrkFilter = $tbl_actual_act->CurrentFilter;

	// Set up filter (Sql Where Clause) and get Return Sql
	// Sql constructor in tbl_actual_act class, tbl_actual_actinfo.php

	$tbl_actual_act->CurrentFilter = $sWrkFilter;
	...
	if ($DeleteRows) {
		foreach ($rsold as $row) {
			$DeleteRows = $tbl_actual_act->Row_Deleting($row);
			if (!$DeleteRows) break;
		}
	}
	if ($DeleteRows) {
		...
		foreach ($rsold as $row) {
			...
			$sThisKey .= $row['a_actid'];
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$DeleteRows = $conn->Execute($tbl_actual_act->DeleteSQL($row)); // Delete
			$conn->raiseErrorFn = '';
			if ($DeleteRows === FALSE)
				break;
			if ($sKey <> "") $sKey .= ", ";
	...
	if ($DeleteRows) {
		foreach ($rsold as $row) {
			$tbl_actual_act->Row_Deleted($row);
		}	
	}
	return $DeleteRows;
}
