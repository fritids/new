...
<p><span class="edge"> Students Search <br>
  <br>
...
<input type="text" name="x_s_usrname" id="x_s_usrname" title="" size="15" maxlength="45" value="<?php echo $tbl_students->s_usrname->EditValue ?>"<?php echo $tbl_students->s_usrname->EditAttributes() ?>>
	...
	<tr class="ewTableRow">
		<td class="ewTableHeader">Instructor Name</td>
		<td<?php echo $tbl_students->i_instructid->CellAttributes() ?>><span class="ewSearchOpr">=<input type="hidden" name="z_i_instructid" id="z_i_instructid" value="="></span></td>
		<td<?php echo $tbl_students->i_instructid->CellAttributes() ?>><span class="edge">
...
if (is_array($tbl_students->i_instructid->EditValue)) {
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($tbl_students->i_instructid->AdvancedSearch->SearchValue) == strval($arwrk[$rowcntwrk][0])) ? " selected" : "";	
?>
...
<!--
var f = document.ftbl_studentssearch;
...
function RenderRow() {
	...
	if ($tbl_students->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		$sSqlWrk = "SELECT `i_instructorid`, `i_first_name`, `i_last_name` FROM `tbl_instructors`";
		if (trim(strval($tbl_students->i_instructid->AdvancedSearch->SearchValue)) == "") {
			$sSqlWrk .= " WHERE 0=1";
		} else {
			$sSqlWrk .= " WHERE `i_instructorid` = " . ew_AdjustSql($tbl_students->i_instructid->AdvancedSearch->SearchValue) . "";
		}
