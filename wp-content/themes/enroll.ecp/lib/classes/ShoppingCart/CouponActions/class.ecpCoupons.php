...
class ecpCoupons
{
	...
	public function decreaseCouponUsage($coupon_id)
	{
		...
		//$this -> db -> update("wp_ecp_coupons", array("coupon_usage" => "coupon_usage-1"), array("coupon_id" => $coupon_id));
		$this -> qresult = $this -> db -> query($this -> query);
		return $this -> qresult;
	}
	...
	public function checkCouponExist($coupon_code)
	{
		$this -> query = "SELECT coupon_id, coupon_price 
							FROM wp_ecp_coupons 
							WHERE BINARY coupon_code = '$coupon_code' 
							AND coupon_usage > 0
							AND CURDATE() >= coupon_startdate
							AND CURDATE() <= coupon_enddate
							AND coupon_enabled
							LIMIT 1";
		$this -> qresult = $this -> db -> get_results($this -> query, ARRAY_A);
		return $this -> qresult;
	...
	public function getAllProductsByCouponID($coupon_id)
	{
		$this -> query = "SELECT product_id FROM wp_ecp_coup_prod WHERE coupon_id = '$coupon_id'";
		$this -> qresult = $this -> db -> get_results($this -> query, ARRAY_A);
		return $this -> qresult;
	}
	
	
	public function checkCouponExistObject($coupon_code)
	{
		$this -> query = "SELECT coupon_id, coupon_price 
							FROM wp_ecp_coupons 
							WHERE BINARY coupon_code = '$coupon_code' 
							...
							AND coupon_enabled
							LIMIT 1";
		$this -> qresult = $this -> db -> get_results($this -> query, ARRAY_N);
		return $this -> qresult;
	}
	
	public function getAllProductsByCouponIDObject($coupon_id)
	{
		$this -> query = "SELECT product_id FROM wp_ecp_coup_prod WHERE coupon_id = '$coupon_id'";
		$this -> qresult = $this -> db -> get_results($this -> query);
		return $this -> qresult;
	}
}
?>