...
function ecp_donations(){
	
	/*
	
	(
    [id] => 754980
    [proposalURL] => http://qa.donorschoose.org/project/mrs-ms-wolfpack-wants-to-get-smart-abo/754980/?utm_source=api&amp;utm_medium=feed&amp;utm_content=bodylink&amp;utm_campaign=DONORSCHOOSE
    [fundURL] => https://qasecure.donorschoose.org/donors/givingCart.html?proposalid=754980&amp;donationAmount=&amp;utm_source=api&amp;utm_medium=feed&amp;utm_content=fundlink&amp;utm_campaign=DONORSCHOOSE
    [imageURL] => http://qa-cdn.donorschoose.net/images/user/uploads/small/u1111705_sm.jpg?timestamp=1334116560112
    [title] => Mrs. M&#039;s Wolfpack Wants to get Smart About Learning
    [shortDescription] => Teens spend most of their time outside of school surrounded by some type of technology. From the Internet to texting devices, this generation has seen how technology works in their daily lives -...
    [fulfillmentTrailer] => My students need a Smart board with projector and 34 student response remotes.
    [percentFunded] => 15
    [costToComplete] => 7427.87
    [matchingFund] => stdClass Object
        (
            [matchingKey] => 
            [name] => 
            [logoURL] => 
    ...
    [totalPrice] => 8700.52
    [freeShipping] => true
    [teacherName] => Mrs. Murphy
    [gradeLevel] => stdClass Object
        (
            [id] => 4
            [name] => Grades 9-12
        )
    ...
    [schoolName] => Ridgeview High School
    [city] => Bakersfield
    [zip] => 93313-9331
    [state] => CA
    [latitude] => 35.278972000000000
    [longitude] => -119.056827000000000
    ...
    [expirationDate] => 2012-07-17
    [fundingStatus] => needs funding
)
	*/
	//error_reporting(E_ALL);
//ini_set('display_errors', '1');
	require_once IDG_CLASS_PATH."/class.ConnectionHandler.php";
	require_once IDG_CLASS_PATH."/class.Donation.php";
	
	?>
		</form>
		<script type="text/javascript">
			jQuery(function(){
				jQuery(".IDGL_Wrap .button-primary").hide();
			})
		</script>
	<?php

	$options = array(
		"email" => "jessbrondo@edgeincollegeprep.com",
		"salutation" => "Mrs",
		"first" => "The Edge",
		"last" => " in College Prep",
		"address1" => "28 W 38th Street #2E",
		"city" => "New York",
		"state" => "NY",
		"zip" => "10018"
	);
	...
	if(isset($_GET["don_id"])){
		$project_id=$_GET["don_id"];
		$amount=(int)get_option("donation_amnt",0);
		
		if($amount>0){
			$donation = new Donation($options);
			$res=$donation->_send_donation($amount, $project_id);
			//print_r($res);
			if($res->statusCode=="0"){
				echo "<h2 style='color:red'>The donation was done successfully</h2>
						<a href='".$res->proposalURL."' target='_blank'>Verify!</a>
				";
				update_option( "donation_amnt", 0 );
			}else{
				echo "<h2 style='color:red'>There was an error placing the donation</h2>";
			}
		}else{
			echo "<h2 style='color:red'>Insufficient funds</h2>";
		}
		...
		<div style="padding:10px;">
			...
			<form method="post">
				...
				Cost To Complete : 
				<select name="costToComplete">
					<option <?php if($_POST["costToComplete"]=="1") echo 'selected="selected"' ?> value="1">$99 or less</option>
					<option <?php if($_POST["costToComplete"]=="2") echo 'selected="selected"' ?> value="2">$100-$200</option>
					<option <?php if($_POST["costToComplete"]=="3") echo 'selected="selected"' ?> value="3">$200-$1,000</option>
					<option <?php if($_POST["costToComplete"]=="4") echo 'selected="selected"' ?> value="4">$1,000-above</option>
				</select>
				
				Resource Type:
				<select name="proposalType">
					<option <?php if($_POST["proposalType"]=="1") echo 'selected="selected"' ?> value="1">Books</option>
					<option <?php if($_POST["proposalType"]=="2") echo 'selected="selected"' ?> value="2">Technology</option>
					<option <?php if($_POST["proposalType"]=="3") echo 'selected="selected"' ?> value="3">Supplies</option>
					<option <?php if($_POST["proposalType"]=="4") echo 'selected="selected"' ?> value="4">Trips</option>
					<option <?php if($_POST["proposalType"]=="5") echo 'selected="selected"' ?> value="4">Visitors</option>
					<option <?php if($_POST["proposalType"]=="6") echo 'selected="selected"' ?> value="4">Other</option>
				</select>
				
				Sort By : 
				<select name="sortBy">
					<option <?php if($_POST["sortBy"]=="1") echo 'selected="selected"' ?> value="0">Urgency</option>
					<option <?php if($_POST["sortBy"]=="2") echo 'selected="selected"' ?> value="1">Poverty</option>
					<option <?php if($_POST["sortBy"]=="3") echo 'selected="selected"' ?> value="2">Cost</option>
					<option <?php if($_POST["sortBy"]=="4") echo 'selected="selected"' ?> value="3">Popularity</option>
				</select>
		...
		<?php
		echo "<h2 style='padding:10px; color:#f00;'>$".get_option("donation_amnt",0)." available for donation</h2>";
		echo "<ul>";
		$donation = new Donation($options);
		$proposals = $donation -> get_proposals($amount);
		foreach($proposals as $proposal){
			echo "<li>";
			echo "<img style='float:left' width='50' src='".$proposal->imageURL."' />";
			echo "<div style='float:left;margin-left:20px;'><h3 style='margin-top:0px;'>".$proposal->schoolName."  --- <strong style='color:red;'>".$proposal->costToComplete."</strong> of ".$proposal->totalPrice."</h3>";
			echo "<p>".$proposal->fulfillmentTrailer."</p><a href='".Util::curPageURL()."&don_id=".$proposal->id."' class='button'>Donate</a></div><div style='clear:both;'></div>";
			echo "<hr/></li>";	
		}
		echo "</ul>";
	}
