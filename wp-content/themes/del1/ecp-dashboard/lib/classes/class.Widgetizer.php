							...
							case "Image":
								$finalClass.= 'echo \'
									<div  class="imageWrap" style="min-width:'.$nd->attributes["width"].'px;min-height:'.$nd->attributes["height"].'px;">
										<img class="elem" src="\'.$instance[\''.$nd->getName().'\'].\'"  width="'.$nd->attributes["width"].'" height="'.$nd->attributes["height"].'"  _scalex="'.$nd->attributes["width"].'" _scaley="'.$nd->attributes["height"].'" />
										<div>
							   			<input type="hidden" class="hiddenFld" name="\'.$this->get_field_name( \''.$nd->getName().'\' ) .\'" value="\'.$instance[\''.$nd->getName().'\'].\'" />
