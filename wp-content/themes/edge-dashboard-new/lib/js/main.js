...
function initComponents(node) {
	if(node == null) {
		jQuery(".IDGL_listNode .IDGL_DatePicker input").datepicker({
			showOn : 'button',
		...
		jQuery(".IDGL_Wrap").each(function() {
			...
			if(jQuery(this).find(".tabList").html() == null) {
				jQuery(this).find(".IDGL_TabBegin").each(function(i) {
					...
					{
						tabList += "<li><a href='#" + jQuery(this).attr("id") + "'>" + jQuery(this).attr("title") + "</a></li>";
					}
					else
					{
		...
		node.find(".IDGL_DatePicker input").datepicker({
			showOn : 'button',
			buttonImage : imagesPath + 'calendar.gif',
			buttonImageOnly : true
		});
...
jQuery(function() {
	...
	jQuery.fn.googleMap = function(target, address) {
		...
		if(address != "") {
			...
			var adr = address.split(","); latlng = new google.maps.LatLng(adr[0], adr[1]), zoomlvl = parseInt(adr[2]);
		}
		var myOptions = {
			zoom : zoomlvl,
			center : latlng,
...
function getPlayerFMS(filepath, id){
	return '<embed height="270" width="400" flashvars="stream='+filepath+'" wmode="opaque" allowscriptaccess="always" allowfullscreen="true" quality="high" bgcolor="#000000" name="qplayer" id="qplayer" style="" src="http://edgeincollegeprep.com/portal/wp-content/themes/ecp-dashboard/lib/images/player.swf" type="application/x-shockwave-flash">';
}
