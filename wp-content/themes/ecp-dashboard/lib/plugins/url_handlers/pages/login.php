...
//    $_SESSION['auth']='test';
    ...
    if(isset($_SESSION['auth'])){
        $code = $_GET['code'];
        $state = $_GET['state'];
        if($state == $_SESSION['auth'])
        {
            $response = fopen("https://auth.casenex.com/auth/access_token?client_id=edge_id&client_secret=2816fhsO49fD2h3yd78f9sa439gEfkf2lhni&code=".$code."&redirect_uri=http://www.edgeincollegeprep.com/portal","r");
...
<form method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>" id="loginform" name="loginform">
    ...
    <p>
        <a href="https://auth.casenex.com/auth/authorize?client_id=edge_id&redirect_uri=<?php echo htmlentities('http://sandbox.edgeincollegeprep.com/oauth_test.php/'); ?>&state=<?php echo $authorization_key; ?>">
            Login with CaseNEX
        </a>
    </p>
	<p class="submit">
