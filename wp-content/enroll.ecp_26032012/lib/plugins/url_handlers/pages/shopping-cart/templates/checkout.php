...
foreach($ecp_cart -> getAllProducts() as $i => $product)
{
	...
	if($product -> discounted_price)
	{
		...
		{
			$payment -> addLineItem(($i + 1), htmlentities(StringUtils::shortenString("{$product -> name}", 25), ENT_QUOTES), htmlentities(StringUtils::shortenString("{$product -> desc}", 25), ENT_QUOTES), "1", "{$product -> discounted_price}", "Y");
		}
	}
	else 
	{
		...
		if(!is_null($product -> name))
		{
			$payment -> addLineItem(($i + 1), htmlentities(StringUtils::shortenString("{$product -> name}", 25), ENT_QUOTES), htmlentities(StringUtils::shortenString("{$product -> desc}", 25), ENT_QUOTES), "1", "{$product -> price}", "Y");
		}
	}
}
