...
// Get test sections
$query = "SELECT `sections`.`id`,`sections`.`name` FROM ".ECP_MCT_TABLE_SECTIONS." `sections`
	LEFT JOIN ".ECP_MCT_TABLE_USER_ANSWERS." `answers` ON `answers`.`section_id` = .`sections`.`id` 
	AND `answers`.`user_id`=%d
	WHERE `sections`.`test_id`=%d
	AND `answers`.`answers` IS NULL
